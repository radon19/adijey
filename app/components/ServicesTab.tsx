...
export default function ServicesTab() {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: "2.5rem" }} className="svc-grid">
        ...
        {services.map(s => {
          ...
          return (
            <button key={s.id} onClick={() => setActive(s.id)} style={{
              ...
              textAlign: "left",
              background: on ? "rgba(201,154,64,0.1)" : "var(--bg-card)",
              border: "1px solid",
      ...
      {/* Panel */}
      <div style={{ padding: "2.5rem", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "6px" }}>
        <div style={{ fontFamily: "var(--font-ui)", fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.25em", textTransform: "uppercase", color: "var(--gold)", marginBottom: "0.5rem" }}>
          {cur.label}
        </div>
        <h3 style={{ fontFamily: "var(--font-display)", fontSize: "1.9rem", fontWeight: 700, color: "var(--text-primary)", marginBottom: "2rem" }}>
          {cur.headline}
        </h3>
        ...
        <Link href="/contact" className="btn-gold" style={{ marginTop: "2.5rem", display: "inline-flex", padding: "0.75rem 1.75rem", fontSize: "0.78rem" }}>
